...
//-------------------------------------------JOYSTICK DA ESQUERDA MOVIMENTOS DO ROBÔ----------------------------------
 


  void blinkverde(void *pvParameters){
    for(;;){
      //== LEFT JOYSTICK - UP ==//
      if (ctl->axisY() <= -25) {
...
//-------------------------------------------JOYSTICK DA DIREITA - MOVIMENTOS DE GARRAS ----------------------------------
...
void setup() {
  ...
  //xTaskCreatePinnedToCore(nome da função, "tarefa",Tamanho da pilha(usa muito 10000), parâmetros (nesse caso não vamos usar),  
  //Prioridade (Quanto maior o valor maior a prioridade), alteração Manual, Núcleo usado (A esp32 só tem 2 núcleos 0 ou 1));
  xTaskCreatePinnedToCore(blinkverde, "tarefa1", 10000, NULL, 0, &tarefa1, 0);
  xTaskCreatePinnedToCore(blinkamarelo, "tarefa2", 10000, NULL, 1, &tarefa2, 1);
